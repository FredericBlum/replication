...
# Not really used right now, can be skipped - also needs fixing
dft_gr <- countBy(groupingVar='cardinal', normBy=mv, dataSource=df1) %>%
  # Average across words
  group_by(word) %>% 
  summarise(across(where(is.double), mean)) %>%
  # Odds for all cardinals
  pivot_longer(!word, names_to='cardinal', values_to='prop') %>% 
  mutate(odds=odds(prop)) %>% 
  left_join(ipa, by='cardinal', relationship='many-to-many') %>% 
  # Compute log odds ratio for each cardinal
  group_by(cardinal) %>% mutate(card_odds=odds(mean(prop))) %>% 
  ungroup() %>%
  mutate(logOdds = log(odds/card_odds), vars=c_across(myvar)) %>% 
  select(word, cardinal, logOdds, vars) %>% 
  distinct(word, cardinal, logOdds, vars)
